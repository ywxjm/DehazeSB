...
def get_clip_score_from_feature(tensor, text_features):
	...
	for i in range(tensor.shape[0]):
		...
		similarity_1 = (image_features/image_nor) @ (text_features/nor).T
		similarity = (100.0 * similarity_1).softmax(dim=-1)
		# similarity = (100.0 * similarity_1)
		probs = similarity
		prob = probs[0][0]
		score =score + prob
...
def get_clip_score_from_feature_only_hazy(x_fake, x_real, text_features):
	score = 0
	for i in range(x_fake.shape[0]):
		image_fake = img_resize(x_fake[i])
		image_f = clip_normalizer(image_fake.reshape(1, 3, 224, 224))

		image_f_features = model.encode_image(image_f)
		image_f_nor = image_f_features.norm(dim=-1, keepdim=True)

		image_real = img_resize(x_real[i])
		image_r = clip_normalizer(image_real.reshape(1, 3, 224, 224))

		image_r_features = model.encode_image(image_r)
		image_r_nor = image_r_features.norm(dim=-1, keepdim=True)

		nor = text_features.norm(dim=-1, keepdim=True)

		similarity_f = (100.0 * (image_f_features / image_f_nor) @ (text_features / nor).T)
		similarity_r = (100.0 * (image_r_features / image_r_nor) @ (text_features / nor).T)
		similarity = torch.cat((similarity_f, similarity_r), dim=1)
		...
		prob = probs[0][0]
		score = score + prob
	score = score / x_fake.shape[0]
	return score


class L_clip_from_feature(nn.Module):
	def __init__(self, only_hazy=False):
		super(L_clip_from_feature,self).__init__()
		self.only_hazy = only_hazy
		for param in self.parameters(): 
			param.requires_grad = False
  
	def forward(self, x_fake, x_real, text_features):
		if self.only_hazy:
			k1 = get_clip_score_from_feature_only_hazy(x_fake, x_real, text_features)
		else:
			k1 = get_clip_score_from_feature(x_fake, x_real, text_features)
		return k1
