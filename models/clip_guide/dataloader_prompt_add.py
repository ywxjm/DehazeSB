...
def populate_train_list(lowlight_images_path,normallight_images_path,overlight_images_path=None):
	...
	if overlight_images_path!=None:
		...
		for extension in image_extensions:
			# 使用glob模块获取所有匹配的图片文件
			image_list_lowlight += glob.glob(lowlight_images_path + extension)
			image_list_normallight += glob.glob(normallight_images_path + extension)
...
class lowlight_loader(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if ("train_clear" in data_lowlight_path):
			label=torch.from_numpy(np.array(1))
		else:
			label=torch.from_numpy(np.array(0))
...
class lowlight_loader_only_gazy(data.Dataset):
	...
	def __getitem__(self, index):
		...
		# else:
		# 	label=torch.from_numpy(np.array(0))
		if ("train_hazy" in data_lowlight_path):
			label = torch.from_numpy(np.array(1))
		else:
			label = torch.from_numpy(np.array(0))
