...
class dehazesbtestModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_hfp', type=float, default=1.0, help='weight for GAN loss：DCP')
        parser.add_argument('--lambda_dcp', type=float, default=1.0, help='weight for GAN loss：DCP')
        ...
        parser.add_argument('--if_ssim',type=util.str2bool,default=False,help='whether or not use additional ssim regularization')
        parser.add_argument('--lambda_ssim',type=float,default=1.0,help='weight for ssim loss')
        parser.add_argument('--ssim_idt',type=util.str2bool,default=False, help='if use ssim for identity mapping, only set it to be true if using ssim regularization')
    ...
    def forward(self):
        ...
        if self.opt.phase == 'test' or self.opt.validation_phase == True:  ### use this part when testing or validation during training 
            ...
            with torch.no_grad():
                self.netG.eval()
                for t in tqdm.tqdm(range(self.opt.num_timesteps)):
    ...
    def compute_D_loss(self): ## discriminator to make real_B to be real, fake_B to be fake, Use GANloss
        ...
        # self.time_embed = self.time_embed.repat(1, )
        ...
        self.loss_D = (self.loss_D1 + self.loss_D) * 0.5
        return self.loss_D
    ...
    def compute_G_loss(self):
        ...
        self.loss_G = self.loss_G_GAN + self.opt.lambda_SB * self.loss_SB + self.opt.lambda_NCE * loss_NCE_both + \
                      self.loss_SSIM * self.opt.lambda_ssim + self.loss_CLIP + self.loss_HFP_all * 0.5 \
                      + self.loss_DCP * 0.5
        return self.loss_G
