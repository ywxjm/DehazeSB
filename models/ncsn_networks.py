    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        if no_antialias:
            ...
            self.model_main.append(
                ConvBlock_cond(input_nc, ndf, 4 * ndf, kernel_size=kw, stride=1, padding=padw, use_bias=use_bias))
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if no_antialias:
                ...
                self.model_main.append(
                    ConvBlock_cond(ndf * nf_mult_prev, ndf * nf_mult, 4 * ndf, kernel_size=kw, stride=1, padding=padw,
...
class NLayerDiscriminator_ncsn_cond(nn.Module):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(NLayerDiscriminator_ncsn_cond, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
    ...
    def forward(self, input, t_emb, input2=None, layers=[], encode_only=False):
        """Standard forward."""
        t_emb = self.t_embed(t_emb)
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model_main):
                feature = layer(feature, t_emb)
...
#         )
#
#     def forward(self, input, t_emb, layers=[], encode_only=False):
#         t_emb = self.t_embed(t_emb)
#         if len(layers) > 0:
#             feat = input
#             feats = []
...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        self.model = nn.Sequential(*model)  ## downsample blocks ###  contain 12 layers
        self.model_upsample = nn.Sequential(*model_upsample)
        mapping_layers = [PixelNorm(),
                      nn.Linear(self.ngf*4, self.ngf*4), #(b,4*ngf) - (b,4*ngf)
                      nn.LeakyReLU(0.2)]
        for _ in range(opt.n_mlp): #n_mlp =3
            mapping_layers.append(nn.Linear(self.ngf*4, self.ngf*4)) #(b,4*ngf) - (b,4*ngf)
            mapping_layers.append(nn.LeakyReLU(0.2))
        self.z_transform = nn.Sequential(*mapping_layers) # total: (b,4 * ngf) -(b,4 * ngf)

        modules_emb = []
        modules_emb += [nn.Linear(self.ngf,self.ngf*4)] #(b,ngf) - (b,ngf*4)
    ...
    def forward(self, x, time_cond,z,layers=[], encode_only=False): #default layers [0,4,8,12,16]
        z_embed = self.z_transform(z) 
        # print(z_embed.shape)
        temb = get_timestep_embedding(time_cond, self.ngf) #(return: time_cond, ngf) time_cond:1-D tensor b 
        time_embed = self.time_embed(temb) # (return: b,4*temb)
        if len(layers) > 0:
            feat = x
            feats = []
            for layer_id, layer in enumerate(self.model):
            ...
            for layer in self.model_res:
                out = layer(out,time_embed,z_embed) ### embedding time and noise information in the middle part ###
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias) #2 continue convolustion blocks
...
class Downsample(nn.Module):
    ...
    def forward(self, inp):
        self.filt = self.filt.to(inp.device)
        if(self.filt_size == 1):
            if(self.pad_off == 0):
