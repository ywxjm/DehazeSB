...
from .ncsn_networks import ResnetGenerator_ncsn, NLayerDiscriminator_ncsn_cond
from .DSCNet import DSCNet_unet, DSCNet_unetgan
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], opt=None,config = None):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UnetTransGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout, r=10,
                             eps=1e-3)
    elif netG == 'unet_256_UNSB':
        net = UnetGenerator_UNSB(input_nc, output_nc, 3, ngf, norm_layer=norm_layer, use_dropout=use_dropout, r=10,
                             eps=1e-3)
    elif netG == 'UNSB_restormer':
        net = UNSB_Restormer(input_nc, output_nc, ngf)
    elif netG == 'UNSB_NFAnet':
        net = NAFNet(img_channel=3, width=32, enc_blk_nums=[2, 4, 4, 6], dec_blk_nums=[6, 4, 4, 2])
    elif netG == 'stylegan2':
    ...
    ### DSCNET
    elif netG == 'DCSNet_unet':
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        net = DSCNet_unet(n_channels=input_nc, n_classes=output_nc, kernel_size=opt.DSC_kernel,
                         extend_scope=opt.DSC_extend, if_offset=opt.DSC_offset, device=device, number=opt.DSC_number,
                         dim=1, n_blocks=opt.DSC_n_blocks, padding_type=opt.DSE_padding_type, use_dropout=use_dropout)
    elif netG == 'DCSNet_unetgan':
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        net = DSCNet_unetgan(n_channels=input_nc, n_classes=output_nc, kernel_size=opt.DSC_kernel,
                         extend_scope=opt.DSC_extend, if_offset=opt.DSC_offset, device=device, number=opt.DSC_number,
                         dim=1, n_blocks=opt.DSC_n_blocks, padding_type=opt.DSE_padding_type, use_dropout=use_dropout)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None,t_emb_dim=128):
    ...
    elif netD == 'basic_cond_nccl':
        net = NLayerDiscriminator_ncsn_cond(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=no_antialias)
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        if gan_mode == 'lsgan':
            self.loss = nn.MSELoss() ### here is mseloss ###
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        n_downsampling = 2
        for i in range(n_downsampling):  # add downsampling layers 64 - 128- 256
            mult = 2 ** i
            if(no_antialias):
...
class UnetGenerator_UNSB(nn.Module):
    """Create a Unet-based generator"""
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=6, norm_layer=nn.BatchNorm2d, use_dropout=False, r=10,
        ...
        It is a recursive process.
        """
        super(UnetGenerator_UNSB, self).__init__()

        self.ngf = ngf
        # construct unet structure
        middle_unet_block = UnetSkipBlock_Cond(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer,
                                          res_cond=True, n_block=9)  # add the innermost layer
        for i in range(num_downs - 5):  # add intermediate layers with ngf * 8 filters
            unet_block = UnetAlignedSkipBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block,
                                              norm_layer=norm_layer, use_dropout=use_dropout)
        # gradually reduce the number of filters from ngf * 8 to ngf
        ...
        self.z_transform = ZTransform(16 * 16, 8 * ngf, n_out=2)
        self.time_embed = TimeEmbedding(8 * ngf, 8 * ngf)

    def forward(self, x, time_cond, z, layers=[], encode_only=False):
        z_embed = self.z_transform(z)
        temb = get_timestep_embedding(time_cond, self.ngf * 8)
        time_embed = self.time_embed(temb)
        if len(layers) > 0:
            pass

        else:
            out = self.model(x, z_embed, time_embed)
...
class ResnetBlock_cond(nn.Module):
    ...
    def __init__(self, dim, padding_type, use_dropout, temb_dim, z_dim):
        ...
        self.conv_block, self.adaptive, self.conv_fin = self.build_conv_block(dim, padding_type, use_dropout, temb_dim,
                                                                              z_dim)
...
class UnetSkipBlock_Cond(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, res_cond=False, norm_layer=nn.BatchNorm2d,
                 use_dropout=False, n_block=None):
        ...
        """
        super(UnetSkipBlock_Cond, self).__init__()
        ...
        if outermost:
            ...
            for i in range(n_block):
                model += [ResnetBlock_cond(self.inner_nc, padding_type='reflect', use_dropout=use_dropout,
                                                    temb_dim= self.inner_nc, z_dim= self.inner_nc)]
        else:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1, bias=use_bias)
    ...
    def forward(self, x, z_embed, time_embed):

        if self.outermost:
            return self.model()
        elif self.res_cond:
            y = self.model(x, z_embed, time_embed)
...
if __name__ == '__main__':
    ...
    print(A.shape)
    test_conv = UnetGenerator_UNSB(3,3,3,64,True,device,16,1,1,'reflect',False)
